...
SVM.prototype = {
    ...
    train(data, labels) {
        ...
        const C = options.C || 1.0; // C value. Decrease for more regularization
        const tol = options.tol || 1e-4; // numerical tolerance. Don't touch unless you're pro
        const alphatol = options.alphatol || 0; // non-support vectors for space and time efficiency are truncated. To guarantee correct result set this to 0 to do no truncating. If you want to increase efficiency, experiment with setting this little higher, up to maybe 1e-4 or so.
        const maxiter = options.maxiter || 10000; // max number of iterations
        const numpasses = options.numpasses || 10; // how many passes over data with no change before we halt? Increase for more precision.
        const SSCA = options.SSCA || false; // smoothed separable case approximation algorithm
        const UB = options.UB || 0.5;
        ...
        for (const d in this.options.kernel) {
        ...
        if ('kernel' in options) {
            if (typeof kernelType === 'string') {
                ...
                if (kernelType === 'rbf') {
                    const rbfSigma = options.rbfsigma || 0.5;
    ...
    timerKarpathySMO() {
        ...
        const stepsFrequency = this.stepsFrequency;
        this.timerVar = setInterval(() => {
            if (this.passes < this.numpasses && this.iter < this.maxiter) {
                ...
                if (!this.input_transformation) {
                    for (let i = 0; i < weights.length; i++) { drawIntermidiate(this.ctx, weights[i]); }
                }
            } else {
    ...
    karpathySMOtime() {
        ...
        let alphaChanged = 0;
        for (let i = 0; i < this.N; i++) {
            const Ei = this.marginOne(data[i]) - labels[i];
            if ((labels[i] * Ei < -this.tol && this.alpha[i] < C)
                ...
                const Ej = this.marginOne(data[j]) - labels[j];
                ...
                const eta = 2 * this.kernelResult(i, j) - this.kernelResult(i, i) - this.kernelResult(j, j);
                if (eta >= 0) continue;
                ...
                this.alpha[j] = newaj;
                const newai = ai + labels[i] * labels[j] * (aj - newaj);
                this.alpha[i] = newai;

                // update the bias term
                const b1 = this.b - Ei - labels[i] * (newai - ai) * this.kernelResult(i, i)
                    - labels[j] * (newaj - aj) * this.kernelResult(i, j);
                const b2 = this.b - Ej - labels[i] * (newai - ai) * this.kernelResult(i, j)
    ...
    getMaxMargin(data) {
        const margins = this.margins(data);
        let max = 0;
    ...
    marginOne(inst) {
        ...
        let f = 0;
        if (this.karpathy) { f = this.b; } else f = -this.b;
    ...
    toJSON() {
        ...
        json.D = this.D;
        json._parametroB = 'bias: coefficiente additivo del calcolo della SVM';
        json.b = this.b;

        json.kernelType = this.kernelType;
        if (this.kernelType === 'linear') {
            // just back up the weights
            json._parametroW = 'weights: coefficienti moltiplicativi del calcolo della SVM';
            json.w = this.w;
        }
    ...
    takeStep(i1, i2, i, j) {
        ...
        const alph1 = this.alpha[i];
        const alph2 = this.alpha[j];
        const y1 = this.labels[i];
        const y2 = this.labels[j];
        const E1 = this.getE(i);
        const E2 = this.getE(j);
        const s = y1 * y2;
        // Compute L, H via equations (13) and (14)
        const C = this.C; // utiliy variable
        let L; let H;
        if (y1 === y2) {
        ...
        const k11 = this.kernelResult(i, i);
        const k12 = this.kernelResult(i, j);
        const k22 = this.kernelResult(j, j);
        ...
        if (eta > 0) {
            ...
            // console.info("eta <0");
            const f1 = y1 * (E1 + this.b) - alph1 * this.kernelResult(i, i) - s * alph2 * this.kernelResult(i, j);
            const f2 = y2 * (E2 + this.b) - s * alph1 * this.kernelResult(i, j) - alph2 * this.kernelResult(j, j);
            const L1 = alph1 + s * (alph2 - L);
            const H1 = alph1 + s * (alph2 - H);
            const Lobj = L1 * f1 - L * f2 + 0.5 * L1 * L1 * this.kernelResult(i, i) + 0.5 * L * L * this.kernelResult(j, j) + s * L * L1 * this.kernelResult(i, j);
            const Hobj = H1 * f1 + H * f2 + 0.5 * H1 * H1 * this.kernelResult(i, i) + 0.5 * H * H * this.kernelResult(j, j) + s * H * H1 * this.kernelResult(i, j);

            if (Lobj < Hobj - this.eps) { a2 = L; } else if (Lobj > Hobj + this.eps) { a2 = H; } else { a2 = alph2; }
            // this.alpha[i] = value; //risetto il valore a quello di prima
        }
        // console.info("a2-alph2 = "+Math.abs(a2-alph2)+" < "+this.eps*(a2+alph2+this.eps)+" ?");
        if (Math.abs(a2 - alph2) < this.eps * (a2 + alph2 + this.eps)) { return 0; }
        // console.info("No,you're done");
        a1 = alph1 + s * (alph2 - a2);

        // console.info("updating");
        // Update threshold to reflect change in Lagrange multipliers
        const b1 = this.b + E1 + y1 * (a1 - alph1) * this.kernelResult(i, i) + y2 * (a2 - alph2) * this.kernelResult(i, j);
        const b2 = this.b + E2 + y1 * (a1 - alph1) * this.kernelResult(i, j) + y2 * (a2 - alph2) * this.kernelResult(j, j);
        this.b = 0.5 * (b1 + b2);
    ...
    notAtBoundsAlpha() {
        ...
        for (let i = 0; i < this.alpha.length; i++) {
            if (!this.isAtBounds(this.alpha[i])) { indexes.push(i); }
        }
        return indexes;
    },
    ...
    examineExample(i2, i) {
        ...
        //* **************
        const y2 = this.labels[i];
        const alph2 = this.alpha[i];
        const E2 = this.getE(i);
        const r2 = E2 * y2;
        let i1;
        if ((r2 < -this.tol && alph2 < this.C) || (r2 > this.tol && alph2 > 0)) {
            const indexes = this.notAtBoundsAlpha();
            if (indexes.length > 1) { // number of non-zero & non-C alpha > 1
                const index = this.getMaxStepAlpha(i);
            ...
            for (let j = rand; counter < indexes.length; j++) { // loop over all non-zero and non-C alpha, starting at a random point
                ...
                if (this.takeStep(i1, i2, j, i)) { return 1; }
    ...
    SMO() {
        ...
        while ((numChanged > 0 || examineAll) && this.iter < this.maxiter) { // outer loop
            ...
            if (examineAll === 1) { examineAll = 0; } else if (numChanged === 0) { examineAll = 1; }
            this.iter++;
        }
    ...
    marginSSCA(inst, data, alpha, labels) {
        let f = -this.b;
    ...
    SSCA(D, options) {
        ...
        const alpha = utils.array.copyArray(this.alpha);
        const labels = utils.array.copyArray(this.labels);
        const data = utils.array.copyArray(this.data);
